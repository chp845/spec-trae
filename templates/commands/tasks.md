...
在继续之前, 你**必须**考虑用户输入(如果不为空). 用户可以提供**任意数量**的额外参考文档或指令来指导任务扩写, 例如: 
- **多重参考文档**: 同时提供多个本地路径、代码文件、设计文档或外部链接.
- **特定扩写指令**: 如 "为所有数据库操作增加性能监控任务" 或 "包含详细的部署脚本编写任务".
- **任务粒度偏好**: 请求更细致或更粗略的任务拆分.

**输入示例**:
- `docs/api-spec.md docs/design-assets.pdf legacy/schema.sql --granularity fine` (同时对比 API 规范、设计资产和旧版 SQL 架构进行细粒度拆分)
- `"参考 docs/prd.md 和 docs/ux-flow.json 扩写 US2 的前端交互任务"` (联合 PRD 和 UX 流程定义特定故事的任务)
- `https://api.docs.com/v2 research/benchmarks.md --include-optimization` (结合外部 API 文档和内部基准测试报告增加优化任务)

## 概述

1. **设置**: 从仓库根目录运行 `{SCRIPT}` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表. 所有路径必须是绝对路径. 同时也应考虑 $ARGUMENTS 中提到的**所有外部文档**. 对于参数值中的单引号如 "I'm Groot", 使用转义语法: 例如 'I'\''m Groot'(或尽可能使用双引号: "I'm Groot").

2. **加载设计文档**: 从 FEATURE_DIR 及用户输入中读取: 
   - **必需**: plan.md(技术栈、库、结构), spec.md(带优先级的用户故事)
   - **可选**: data-model.md(实体), contracts/(API 端点), research.md(决策), quickstart.md(测试场景)
   - **外部输入**: 任何通过 $ARGUMENTS 提供的**所有补充参考文档** (支持多文档联合输入, 如 PRD、架构图描述、外部 API 定义等).
   - 注意: 并非所有项目都有所有文档. 基于所有可用内容及用户提供的**多源补充信息**生成/扩写任务.

3. **执行任务生成与扩写工作流**:
   - 加载 plan.md 并提取技术栈、库、项目结构
   - 加载 spec.md 并提取带优先级的用户故事(P1、P2、P3 等)
   - **多文档集成**: 从用户提供的**多个补充文档**中交叉提取技术实现步骤、安全要求或特定的非功能性任务.
   - 如果存在 data-model.md: 提取实体并映射到用户故事
   - 如果存在 contracts/: 映射端点到用户故事
   - 如果存在 research.md: 提取设置任务的决策
   - **多源任务扩写**: 结合**所有输入文档**, 将高级用户故事细化为可立即执行的原子任务. 确保每个任务都通过**多文档比对**包含了最详尽的细节 (如跨文档定义的特定字段、一致性约束、性能指标).
   - 按用户故事生成任务(参见下面的任务生成规则)
   - 生成显示用户故事完成顺序的依赖关系图
   - 为每个用户故事创建并行执行示例
   - 验证任务完整性(每个用户故事都有所有必需任务, 可独立测试, 且符合**所有参考文档**的联合约束)
