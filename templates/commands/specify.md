...
用户在触发消息中 `/speckit.specify` 后输入的文本**就是**功能描述. 假设你始终可以在本次对话中访问它, 即使下面字面上显示 `{ARGS}`. 除非用户提供了空命令, 否则不要要求用户重复.

**需求提取源**: 
- 如果用户在输入中指定了一个或多个文件路径(如 "从 docs/req1.pdf, docs/req2.md 提取"), 你**必须**读取并分析所有指定的文档内容作为主要需求源. 
- 你需要整合来自多个源的信息, 识别其中的关联、补充或潜在冲突. 
- 如果未指定文件, 则将用户的自然语言描述作为唯一需求源. 

基于该功能描述或所有指定文档的整合内容, 执行以下操作:
...
4. 遵循此执行流程:
    ...
    1. 从输入或所有指定的源文档解析用户需求
       - 如果用户提供了文件路径(支持多个), 依次读取并解析这些文件. 
       - 整合所有文档中的核心功能、约束、非功能性需求. 
       - 处理文档间的冲突: 优先遵循用户最新的补充说明, 其次遵循更具体的文档. 
       - 如果输入和文档都为空: 错误 "未提供功能描述或源文档"
    2. 从描述或整合后的文档中提取关键概念
       识别: 参与者、操作、数据、约束
    3. 对于不明确的方面: 
       - 基于上下文和行业标准做出有根据的猜测
