...
description: Perform cross-artifact consistency and quality analysis on spec.md, plan.md, and tasks.md after task generation. Identify inconsistencies, redundancies, ambiguities, and under-specified items.
license: MIT
...
**章程权威**：项目章程（`.specify/mem/constitution.md`）在此分析范围内是**不可协商的**。章程冲突自动为严重问题，需要调整规范、计划或任务——而不是稀释、重新解释或默默忽略原则。如果原则本身需要更改，那必须在 `/speckit.analyze` 之外的单独、明确的章程更新中进行。

## 执行步骤

### 1. 初始化分析上下文

从仓库根目录运行一次 `.specify/scripts/bash/create-new-feature.sh --json` 并解析 JSON 以获取 FEATURE_DIR 和 AVAILABLE_DOCS。推导绝对路径：
...
- 加载 `.specify/mem/constitution.md` 进行原则验证

### 3. 构建语义模型

创建内部表示（输出中不包含原始制品）：

- **需求清单**：每个功能和非功能需求，带有稳定键
- **用户故事/操作清单**：带有验收标准
...
| ID | 类别 | 严重性 | 位置 | 摘要 | 建议 |
|----|------|--------|------|------|------|
| A1 | 重复 | 高 | spec.md:L120-134 | 两个相似需求... | 合并表述 |
...
询问："你想让我为前 N 个问题建议具体的修复编辑吗？"（不要自动应用它们）。
